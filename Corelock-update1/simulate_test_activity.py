"""
CoreLock - Generador de actividad de prueba (SOLO PARA TESTING)
==================================================================
Este script NO es malware. Genera actividad benigna pero que IMITA
los patrones que CoreLock busca detectar, para que puedas confirmar
en vivo que las alertas realmente aparecen.

Uso:
    python simulate_test_activity.py cpu       -> genera carga sostenida de CPU
    python simulate_test_activity.py scan      -> simula conexión a "muchas IPs"
    python simulate_test_activity.py port      -> abre un listener en puerto riesgoso (4444)
"""
...
def simulate_cpu_load(duration=100):
    """Genera carga de CPU sostenida y real (no simulada) para probar
    la detección de CPU sostenida del miner_detector."""
...
def simulate_risky_port_listener(port=4444, duration=60):
    """Abre un socket escuchando en un puerto típico de Stratum/backdoor
    para que network_guard.py lo detecte al ver una conexión ahí.
    No envía ni recibe datos reales, solo abre el puerto."""
...
def simulate_ip_scan_pattern():
    """Simula el patrón de 'contactar muchas IPs distintas rápido' conectando
    a varios servidores DNS públicos conocidos (tráfico real e inofensivo,
    solo para generar el PATRÓN que dispara la alerta de escaneo)."""
    test_ips = [

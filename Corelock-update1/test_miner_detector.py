"""
Tests automáticos para miner_detector.py

Usan mocks (procesos falsos simulados) para verificar que la lógica de
detección funciona correctamente, SIN necesitar ejecutar un minero real
ni esperar los 90 segundos de umbral real.

Correr con:
    python -m pytest test_miner_detector.py -v

o simplemente:
    python test_miner_detector.py
"""
...
class TestSpoofedSystemProcess(unittest.TestCase):
    ...
    def test_legit_explorer_in_windows_not_flagged(self):
        # Regresión: bug real detectado en producción. explorer.exe vive
        # en C:\Windows (NO en System32) y estaba siendo marcado como
        # falso positivo, causando que se matara en loop infinito.
        proc = FakeProcess(
            pid=2004, name="explorer.exe",
            exe_path="C:\\Windows\\explorer.exe"
        )
        self.assertFalse(md.is_spoofed_system_process(proc))

    def test_fake_explorer_outside_windows_still_flagged(self):
        # El fix no debe volverse permisivo de más: un explorer.exe
        # falso en otra carpeta sigue debiendo detectarse.
        proc = FakeProcess(
            pid=2005, name="explorer.exe",
            exe_path="C:\\Users\\test\\AppData\\Local\\Temp\\explorer.exe"
        )
        self.assertTrue(md.is_spoofed_system_process(proc))
